...
Com base no estudo original de [@Cortez2009ModelingWP], e com intuito de permitir observar os atributos de forma mais intuitiva, somos capazes de estabelecer as seguintes unidades de medida e domínios para cada um dos atributos.

```{r echo=FALSE}
fixed_acidity <- c("g(ácido tartárico)/dm$^3$", "$\\mathfrak{R}$")
volatile_acidity <- c("g(ácido acético)/dm$^3$", "$[0,1.2]$")
citric_acid <- c("g/dm$^3$", "$\\mathfrak{R}$")
residual_sugar <- c("g/dm$^3$", "$\\mathfrak{R}$")
chlorides <- c("g(cloreto de sódio)/dm$^3$", "$\\mathfrak{R}$")
free_sulfur_dioxide <- c("mg/dm$^3$", "$\\mathfrak{R}$")
total_sulfur_dioxide <- c("mg/dm$^3$", "$[0,200]$")
density <- c("g/cm$^3$", "$\\mathfrak{R}$")
pH <- c("sem unidade", "[0,14]")
sulphates <- c("g(sulfato de potássio)/dm$^3$", "$\\mathfrak{R}$")
alcohol <- c("vol.%", "$[0,100]$")
quality <- c("sem unidade", "$\\{0,1,2,\\cdots,10\\}$")
col_names_all = c("Fixed Acidity", "Volatile Acidity", "Citric Acid", "Residual Sugar", "Chlorides", "Free Sulfur Dioxide", "Total Sulfur Dioxide", "Density", "pH", "Sulphates", "Alcohol", "Quality")
main_df <- data.frame(fixed_acidity,volatile_acidity,citric_acid,residual_sugar,chlorides,free_sulfur_dioxide,total_sulfur_dioxide,density,pH,sulphates,alcohol,quality)
names(main_df) <- col_names_all
knitr::kable(t(main_df), col.names=c("Unidade de Medida","Domínio"),caption="Unidades de medida e domínios considerados")
```
